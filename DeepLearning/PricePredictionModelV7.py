...
class RegressionModel(nn.Module):
    def __init__(self, input_size, dropout_prob = 0.5):
        ...
        self.regression = nn.Sequential(
            ...
            nn.BatchNorm1d(1024),
            nn.Dropout(dropout_prob),
            nn.Linear(1024, base_neuron_count),
            nn.Dropout(dropout_prob),
            nn.ReLU(),
            nn.Linear(base_neuron_count, base_neuron_count),
            nn.ReLU(), # 8
            ...
            nn.ReLU(),
            nn.Linear(base_neuron_count, base_neuron_count),
            nn.Dropout(dropout_prob), # 16
            nn.Linear(base_neuron_count, 128),
            nn.ReLU(),
...
class PricePredictionModelV7(nn.Module):
    def __init__(self, tabular_data_size, params):
        super(PricePredictionModelV7, self).__init__()
        # Inception
